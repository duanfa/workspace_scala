...
import com.st.ufsCluster.etl.feature.old.FeatureUtil
/**
 * @author socket
 */
case class FeatureInfo(index: String, word: String, uv: Int, pv: Int, firstKind: String, secondKind: String)
object FeatureETLPercent {
  ...
  def main(args: Array[String]) {
    ...
    val recipeMapAll = sample.flatMap { x =>
      {
        x._2.toSeq
      }
    }.aggregate(scala.collection.mutable.Map.empty[String, Int])(seqOp, combop).map(x => (x._2, x._1))

    var wordSize = recipesArrayBc.value.size;
    val recipeTop = sc.parallelize(recipeMapAll.toSeq).sortByKey(false).take(wordSize)
    ...
    val recipeMapBC = sc.broadcast(top_recipe_orderMap)
    val featureMap = FeatureUtil.initRecipesFeatureMap();
    val featureIndexMap = featureMap.values.map { x => scala.collection.mutable.Set(x.firstKind,x.secondKind) }.reduce(_++_).zipWithIndex.toMap
    var featureSize = featureIndexMap.size;
    val recipesFeatureMapBC = sc.broadcast[scala.collection.mutable.Map[String, FeatureInfo]](featureMap)
    val featureIndexMapBC = sc.broadcast[Map[String, Int]](featureIndexMap)
    val formatResult = sample.map { record =>
      {
        var sum = 0.0
        record._2.map(x => {
          if (x._2 > 0) {
            val recipeId = recipeMapBC.value.get(x._1);
            if (recipeId != None) {
              sum = sum + x._2
            }
          }
        ...
        val array = Array.fill(featureSize)(0.0)
        record._2.map(x => {
          if (x._2 > 0) {
            val featureInfo = recipesFeatureMapBC.value.get(x._1.trim());
            if (featureInfo != None) {
              val firstKindFeatureId = featureIndexMapBC.value.get(featureInfo.get.firstKind.trim());
              val secondKindFeatureId = featureIndexMapBC.value.get(featureInfo.get.secondKind.trim());
              try {

                if (array(firstKindFeatureId.get) == 0) {
                  array(firstKindFeatureId.get) = x._2 / sum
                } else {
                  array(firstKindFeatureId.get) = array(firstKindFeatureId.get) + (x._2 / sum)
                }
                if (array(secondKindFeatureId.get) == 0) {
                  array(secondKindFeatureId.get) = x._2 / sum
                } else {
                  array(secondKindFeatureId.get) = array(secondKindFeatureId.get) + (x._2 / sum)
                }
              } catch {
                case e: Exception => println(featureInfo.get.firstKind+" "+featureInfo.get.secondKind)
              }
    ...
    sc.parallelize(featureMap.toSeq).saveAsTextFile("/user/hadoop/ufsCluster/Dir_featureMap")
    formatResult.saveAsTextFile("/user/hadoop/ufsCluster/Dir_formatResult_feature")
  }
