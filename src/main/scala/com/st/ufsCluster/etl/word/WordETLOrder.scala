...
object WordETLOrder {
  ...
  def main(args: Array[String]) {
    ...
    val formatResult = allResult.map { record =>
      {
        ...
        record._2.map(x => {
          if (x._2 > 0) {
            ...
            if (recipeId != None) {
              array(recipeId.get.toInt) = recipeId.get
            }
          }
    ...
    sc.parallelize(recipeMapAll.toSeq).saveAsTextFile("/user/hadoop/ufsCluster/Dir_recipeMapAll_word_order" + recipTopN)
    sc.parallelize(recipeTop).saveAsTextFile("/user/hadoop/ufsCluster/Dir_recipeTop_word_order" + recipTopN)
    sc.parallelize(top_recipe_orderMapBC.value.toSeq).saveAsTextFile("/user/hadoop/ufsCluster/Dir_recipeMapTop_word_order" + recipTopN)
    formatResult.saveAsTextFile("/user/hadoop/ufsCluster/Dir_formatResult_word_order" + recipTopN)
  }
